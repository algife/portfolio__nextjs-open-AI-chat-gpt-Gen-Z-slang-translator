...
import { transcribeFromZ, transcribeToZ } from "./openai/api.calls";
...
export default function TranscriptionBlock() {
  ...
  const writtenTextAnimation = () => {
    ...
    const intv = setInterval(() => {
      ...
      if (i <= maxIndex + charSteps) {
        setText1(placeholderText1.slice(0, i + 1));
        i = i + charSteps;
        return;
      }

      if (i >= placeholderText1.length - 1) {
        // Finish
        i = 0;
        clearInterval(intv);
  ...
  const handleBtnClick = async () => {
    ...
    switch (lastEdited) {
      case 1:
        ...
        transcript = await transcribeToZ(text1);
        setText2(transcript);
        setLastEdited(0);
        break;
      case 2:
        /* Transcribe from Gen-Z slang to standard */
        transcript = await transcribeFromZ(text2);
        setText1(transcript);
        setLastEdited(0);
        break;
      default:
